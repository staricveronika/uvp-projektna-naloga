...
def shrani_osnovne_htmlje(ZACETNA_STRAN, KONCNA_STRAN):
    """Funkcija shrani HTML strani seznama v mapo."""
    os.makedirs(IMENIK_HTML, exist_ok=True) # ne bo napake, če mapa že obstaja
    ...
    for stran in range(ZACETNA_STRAN, KONCNA_STRAN + 1):
        ...
        if html.status_code == 200: # če je zahteva uspešna
            ...
            with open(ime_datoteke, "w", encoding="utf-8") as f:
                f.write(html.text) # vsebina HTML strani kot besedilo
    ...
    for ime_datoteke in os.listdir(IMENIK_HTML): # prebere vse html datoteke v mapi goodreads_html
        if ime_datoteke.endswith(".html"):
            ...
            with open(pot, encoding="utf-8") as f:
                ...
                for link in najdeno:
                    povezave.add("https://www.goodreads.com/book/show/" + link.group(1)) 
                    # sestavimo celoten url  
    return list(povezave) # vrne seznam url-jev do knjig
...
def shrani_strani_posameznih_knjig(povezave):
    ...
    os.makedirs(IMENIK_KNJIG_HTML, exist_ok=True)
    for povezava in povezave:
        ime_datoteke = povezava.split("/")[-1] + ".html" 
        # ime datoteke naj bo zadnji del url-ja
        # razdelimo url po poševnicah in vzamemo zadnji del (ID knjige)
...
def pocisti_besedilo(besedilo):
    ...
    if besedilo:
        ...
        return besedilo
    return None # če je besedilo None ali prazen niz
    ...
    for stran in range(od, do + 1):
        ...
        with open(pot, encoding="utf-8") as f:
            ...
            for najdba in vzorec.finditer(vsebina):
                ...
                naslov = pocisti_besedilo(najdba["naslov"])
                avtor = pocisti_besedilo(najdba["avtor"])
                povp_ocena = float(najdba["povp_ocena"]) # oceno pretvorimo v decimalno število
                st_ocen = int(najdba["st_ocen"].replace(",", "")) # število ocen pretvorimo v celo število
...
def izlusci_podrobne_podatke_iz_knjige(osnovni_podatki):
    ...
    for knjiga in osnovni_podatki: 
        id_knjige = knjiga[0] # osnovni_podatki je seznam tuplov
        naslov = knjiga[1]
        avtor = knjiga[2]
        povp_ocena = knjiga[3]
        st_ocen = knjiga[4]
        ...
        if vzorec:
            ...
            # rezultat je recimo 2004-09-11 00:00:00
            leto_izdaje = datum_v_utc.year # vrnemo samo leto --> torej 2004
        else:
            leto_izdaje = None # če timestamp ni najden
